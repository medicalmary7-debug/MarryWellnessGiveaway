...
function App() {
  ...
  return <div id="top">
    ...
    <main>
      ...
      <section id="enter" className="entry"><div className="entry-copy"><p className="section-label">05 / YOUR NEXT STEP</p><h2>Begin<br /><em>with one.</em></h2><p>Enter for a chance to receive Medical Mary’s Wellness, Your Way package, valued at R700.</p><ul className="entry-meta"><li>Giveaway: <b>Sunday, 27 September 2026 at 12pm – Saturday, 3 October 2026 at 12pm</b></li><li>Location: <b>10 Kloof Road, Observatory, Johannesburg</b></li><li>Eligibility: <b>People aged 18 and over</b></li><li>Entry: <b>No purchase required</b></li></ul><p className="preview-note">TALLY FORM CONNECTION PENDING<br />Official rules, privacy notice, prize restrictions, and jurisdiction requirements apply.</p></div><div className="form-card">{submitted ? <div className="confirmation"><Leaf size={28} /><p className="eyebrow">TRIAL ENTRY RECEIVED</p><h3>You made room<br /><em>for something good.</em></h3><p>This trial confirmed the form flow locally. Once the Tally form URL is connected, submissions will be sent to Tally.</p></div> : <><p className="form-kicker">ENTRY FORM · TRIAL PREVIEW</p><h3>Enter the Reset</h3><p className="form-sub">It only takes a moment.</p><form onSubmit={handleSubmit}><label>Your name<input name="name" required type="text" autoComplete="name" /></label><label>Email address<input name="email" required type="email" autoComplete="email" /></label><label>What are you making room for?<select name="room" required defaultValue=""><option value="" disabled>Select one</option>{['Rest', 'Movement', 'Connection', 'Creativity', 'Self-care'].map(item => <option key={item}>{item}</option>)}</select></label><label className="check"><input required type="checkbox" /> <span>I confirm that I am aged 18 or over and agree to the <a href="#fine-print">official rules</a>.</span></label><label className="check"><input type="checkbox" /> <span>Send me Medical Mary news and thoughtful updates.</span></label><button className="button button-green submit" type="submit">Submit trial entry <ArrowUpRight size={16} /></button></form><p className="fine-notice">Trial mode does not send data externally. Connect the approved Tally form before launch.</p></>}</div></section>
